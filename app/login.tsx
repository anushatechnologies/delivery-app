...
export default function LoginScreen() {
  ...
  // Strictly enforce 10-digit indian numbers
  ...
  const handleSendOtp = async () => {
    ...
    try {
      const status = await authService.checkPhone(`+91${phone}`);
      if (!status.exists) {
        setLoading(false);
        setPopup({
          visible: true,
    ...
    try {
      ...
      const verificationId = await phoneProvider.verifyPhoneNumber(`+91${phone}`, recaptchaVerifier.current!);
      setLoading(false);
      router.push({ pathname: "/otp", params: { phone, verificationId, from: "login" } });
    } catch (error: any) {
      setLoading(false);
          ...
          <Animated.View entering={FadeInUp.delay(350).duration(800).springify()} style={styles.cardWrapper}>
            <View style={styles.card}>
              <View style={styles.cardHeader}>
                ...
                <Text style={styles.cardSubtitle}>Enter your registered mobile number to access your dashboard.</Text>
              </View>
...
const styles = StyleSheet.create({
  ...
  logoWrapper: { position: "relative", marginBottom: 24 },
  logoContainer: { width: 110, height: 110, borderRadius: 35, backgroundColor: "#FFFFFF", justifyContent: "center", alignItems: "center", zIndex: 2, elevation: 12, shadowColor: "#0A8754", shadowOffset: { width: 0, height: 12 }, shadowOpacity: 0.15, shadowRadius: 24, borderWidth: 1, borderColor: "rgba(10, 135, 84, 0.05)" },
  ...
  brandTagline: { fontSize: 16, color: "#64748B", marginTop: 8, fontWeight: "600", letterSpacing: 0.5, textTransform: "uppercase" },
  ...
  cardTitle: { fontSize: 26, fontWeight: "800", color: "#1E293B", marginBottom: 8, letterSpacing: -0.5 },
  cardSubtitle: { fontSize: 15, color: "#64748B", lineHeight: 22, fontWeight: "500" },
  ...
  signupSection: { flexDirection: "row", justifyContent: "center", alignItems: "center", marginTop: 32, gap: 8 },
